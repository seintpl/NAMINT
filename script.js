...
function nogh(ghimg){
  ghimg.parentNode.innerHTML='<i class="bi bi-github"></i> N/A';
  return true;
}

function addlogin(value) {
  all += '<tr><td style="width: 35%;">' + value + '</td><td><a href=\'https://www.google.pl/search?q="' + value + '"\' target=_blank>Google</a></td><td><a href=\'https://www.bing.com/search?q="' + value + '"\' target=_blank>Bing</a></td><td><a href=\'https://yandex.com/search/?text="' + value + '"\' target=_blank>Yandex</a></td><td><a href=\'https://www.google.pl/search?q="' + value + '@gmail.com"+OR+"' + value + '@outlook.com"+OR+"' + value + '@icloud.com"+OR+"' + value + '@yahoo.com"+OR+"' + value + '@protonmail.com"\' target=_blank><i class="bi-google"></i> common emails</a></td><td><a href=\'https://www.facebook.com/' + value + '\' target=_blank><i class="bi-facebook"></i></a></td><td><a href=\'https://x.com/' + value + '\' target=_blank><i class="bi-twitter-x"></i></a></td><td><a href=\'https://instagram.com/' + value + '\' target=_blank><i class="bi-instagram"></i></a></td><td><a href=\'https://www.tiktok.com/@' + value + '\' target=_blank><i class="bi-tiktok"></i></a></td><td><a href=\'https://www.twitch.tv/' + value + '\' target=_blank><i class="bi-twitch"></i></a></td><td><a href=\'https://seintpl.github.io/imagstodon/?u=' + value + '\' target=_blank><i class="bi bi-mastodon"></i></a></td><td><a href=\'https://whatsmyname.app/?q=' + value + '\' target=_blank>whatsmyname</a></td></tr>';
  vg = value + '@gmail.com';
  ...
  grall += '<tr><td style="width: 35%; vertical-align: middle;">' + value + '</td><td style="vertical-align:middle;"><a href=\'https://github.com/"' + value + '"\' target=_blank><img src="https://github.com/' + value + '.png" style="width:80px;" onerror="nogh(this.parentNode)" /></a></td><td><img src="https://s.gravatar.com/avatar/' + md5(vg) + '"></td><td><img src="https://s.gravatar.com/avatar/' + md5(vo) + '"></td><td><img src="https://s.gravatar.com/avatar/' + md5(vi) + '"></td><td><img src="https://s.gravatar.com/avatar/' + md5(vy) + '"></td><td><img src="https://s.gravatar.com/avatar/' + md5(vh) + '"></td><td><img src="https://s.gravatar.com/avatar/' + md5(vm) + '"></td></tr>';
...
function openalltw(a) {
  all += 'window.open(\'https://x.com/search?q=' + a + '&src=typed_query&f=user\',\'' + a + '\');';
}
...
function mashup() {
  ...
  if (f==''||l=='') {document.getElementById("namint_result").innerHTML = '<div style="padding-top: 10px; font-weight:bold;">Enter at least first and last name above.</div>';}
    ...
    all = '<div>Jump to: <a class="btn btn-outline-success btn-sm" href="#login">login patterns</a> <a class="btn btn-outline-success btn-sm" href="#email">email search tools</a> <a class="btn btn-outline-success btn-sm" href="#gravatar">avatar search</a> <a class="btn btn-outline-success btn-sm" href="#unavatarlogin">unavatar login search</a> <a class="btn btn-outline-success btn-sm" href="#unavataremail">unavatar email search</a> <a class="btn btn-outline-success btn-sm" href="#permutator">email permutator</a></div>';
    all += '<div style="padding-top: 10px; font-weight:bold;">Possible name patterns and search links: <span style="float:right;"><button id="export-json" class="btn btn-outline-primary btn-sm" onclick="downloadJson(\'names\')">Export to JSON</button></span></div><div style="border: 1px solid #ddd;"><table class="table" id="names-table"><thead><tr style="display:none;"><th>name</th><th>google</th><th>google_images</th><th>google_phone</th><th>google_email</th><th>google_maps</th><th>google_pdf</th><th>google_docx</th><th>google_xlsx</th><th>bing</th><th>yandex</th><th>yandex_images</th><th>facebook</th><th>x</th><th>tiktok</th><th>flickr</th><th>vk</th><th>other</th></tr></thead><tbody>';
    grall = emall = '<tr><td> </td><td>Github<br />avatar</td><td>Gravatar<br />@gmail.com</td><td>Gravatar<br />@outlook.com</td><td>Gravatar<br />@' + dom1 + '</td><td>Gravatar<br />@' + dom2 + '</td><td>Gravatar<br />@' + dom3 + '</td><td>Gravatar<br />@' + dom4 + '</td></tr>';
    ...
    all += '</table><a id="login"></div><div style="padding-top: 10px; font-weight:bold;">Possible login patterns and search links: <span style="float:right;"><button id="export-json" class="btn btn-outline-primary btn-sm" onclick="downloadJson(\'logins\')">Export to JSON</button></span></div>';
    all += '<div style="border: 1px solid #ddd;"><table class="table" id="logins-table"><thead><tr style="display:none;"><th>login</th><th>google</th><th>bing</th><th>yandex</th><th>common_emails</th><th>facebook</th><th>x</th><th>instagram</th><th>tiktok</th><th>twitch</th><th>mastodon</th><th>whatsmyname</th></tr></thead>';
...
function downloadJson(tablename) {
  ...
  downloadJSONfile(jsonString, 'NAMINT-' + tablename + '-results.json');
}
